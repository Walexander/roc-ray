...
get_by_component = |ecs, components|
    List.walk_with_index components (Set.empty {}) |ids, component, index|
        ...
        if index == 0 then
            ...
            Set.intersection ids Set.from_list(c_ids)
        ...
        when comp is
            [MoveRequest { destination }, Position point] ->
                ...
                { accum &
                    ...
                    path_requests: Dict.insert accum.path_requests id { start: from_cell, goal: destination },
                }
                    ...
                    (path0, new_segment) =
                        ...
                        when curr_segment is
                            ...
                            Ok segment ->
                                ...
                                if start_cell == start and next != end_cell then
                                    ## yes --
...
kill_system = |ecs|
    removals =
        Dict.to_list ecs.killable
        |> List.keep_oks |(id, { dead_frame })|
        ...
        List.keep_oks(
            ...
            |{ id }|
                ...
                |> Result.map2 (Dict.get ecs.killable id) |{ position, explosion }, { lifetime }|
                    List.map(
                        List.range { start: At 0, end: Before explosion.num_particles },
                        |_| {
                            position: position,
    ...
    |> Dict.walk ecs |accum, id, c|
        when c is
            [Path { path, goal }, Occupies { cell }] ->
                when path is
                    [_, dest, ..] if dest != cell and is_occupied dest ->
